...
for file in os.listdir(file_path):
    ...
    for i in range(len(detections['detection_scores'][0])):
        ...
        if score > 0.5:  # Adjust threshold as needed
            ...
            xmin = (float(cx)-float(width)/2)# * img_width
            xmax = (float(cx)+float(width)/2)# * img_width
            ymin = (float(cy)-float(height)/2)# * img_height
            ymax = (float(cy)+float(height)/2)# * img_height
            bboxes_Eff.append([ymin, xmin, ymax, xmax])
            classes_Eff.append(detections['detection_classes'][0][i].numpy())
            scores_Eff.append(score)
    ...
    #print(bboxes_Eff)
    names_Eff = []
    for clas in classes_Eff:
        names_Eff.append(coco_class_names[clas])
    i = 0
    #print('g')
    print(names_Eff)
    while i < len(names_Eff):
        print(i)
        if names_Eff[i] not in household_categories:
            names_Eff.pop(i)
            bboxes_Eff.pop(i)
            scores_Eff.pop(i)
        i += 1
    ...
    while i < len(names_Eff):
        ...
        for index in possible_match_list:
            ...
            print([bboxes_Eff[i][1], bboxes_Eff[i][0], bboxes_Eff[i][3], bboxes_Eff[i][2]])
            if bboxes_Eff[i][1] > bboxes_Eff[i][3]:
                bboxes_Eff[i][1], bboxes_Eff[i][3] = bboxes_Eff[i][3], bboxes_Eff[i][1]
            if bboxes_Eff[i][0] > bboxes_Eff[i][2]:
                bboxes_Eff[i][0], bboxes_Eff[i][2] = bboxes_Eff[i][2], bboxes_Eff[i][0]
            if COCO_x1 > COCO_x2:
                COCO_x1, COCO_x2 = COCO_x2, COCO_x1
            if COCO_y1 > COCO_y2:
                COCO_y1, COCO_y2 = COCO_y2, COCO_y1
            if bboxes_Eff[i][1] > COCO_x1:
                intersect_x1 = bboxes_Eff[i][1]
            else:
                intersect_x1 = COCO_x1
            if bboxes_Eff[i][3] < COCO_x2:
                intersect_x2 = bboxes_Eff[i][3]
            else:
                intersect_x2 = COCO_x2
            if bboxes_Eff[i][0] < COCO_y1:
                intersect_y1 = bboxes_Eff[i][0]
            else:
                intersect_y1 = COCO_y1
            if bboxes_Eff[i][2] > COCO_y2:
                intersect_y2 = bboxes_Eff[i][2]
            else:
            ...
            Eff_area = (bboxes_Eff[i][3] - bboxes_Eff[i][1]) * (bboxes_Eff[i][2] - bboxes_Eff[i][0])
            union = COCO_area + Eff_area
            pos_IoU = intersection/union
            if pos_IoU > 1:
                print("humbug")
            print([intersect_x1, intersect_y1, intersect_x2, intersect_y2])
            print("COCO x: "+str(COCO_x2-COCO_x1))
            print("COCO y: "+str(COCO_y2-COCO_y1))
            print("EFF x: "+str(bboxes_Eff[i][3]-bboxes_Eff[i][1]))
            print("EFF x: "+str(bboxes_Eff[i][2]-bboxes_Eff[i][0]))
            print(COCO_area)
            print(Eff_area)
            print(intersection)
            print(union)
            #print(names_Eff[index])
            print(pos_IoU)
            if pos_IoU > IoU:
                IoU = pos_IoU
                bobby = intersection
...
f = open("C:/Users/aadip/PycharmProjects/benchmarking/Eff_Results/results.txt", "w")
